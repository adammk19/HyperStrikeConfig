...
// Standard layout: 14 buttons, uniform 8px gap between all buttons
// L/D level, R slightly down-right, U under-left of B3
// M1 top-left of U, M2 top-right of U (44x44 modifiers)
...
const STANDARD_LAYOUT = [
  ...
  { label: 'M1', x: 78, y: 130, width: 44, height: 44 },      // index 8: top-left of U
  { label: 'M2', x: 190, y: 142, width: 44, height: 44 },     // index 9: top-right of U

  { label: 'B5', x: 320, y: 8, width: 56, height: 56 },       // index 10
  { label: 'B6', x: 384, y: 20, width: 56, height: 56 },      // index 11
  { label: 'B7', x: 320, y: 72, width: 56, height: 56 },      // index 12
  { label: 'B8', x: 384, y: 84, width: 56, height: 56 }       // index 13
]
...
export function StandardLayout({
  ...
  hasFirmwareMaskRef,
  onButtonClick
}: StandardLayoutProps): React.JSX.Element {
  return (
    <div className="relative" style={{ width: 440, height: 204 }}>
      {STANDARD_LAYOUT.map((btn, i) => (
        <ButtonVisual
          key={i}
          index={i}
          label={btn.label}
